...
//--------------------------------------------------------------------
//	ボタン決定部
#define SFC_CLK	LATCbits.LATC1
#define SFC_PS	LATCbits.LATC2
#define SFC_DAT1	PIN9
#define SFC_SEL	LATCbits.LATC0
#define SFC_DAT2	PIN8
...
|o o o o | o o o )
-----------------
Vcc,CLK,PS,DAT1,DAT2,SEL,GND
*/
// SFCパッド読み取り
//パッドは16ビット(後4ビットはPAD IDですが無視して頭12ビットだけ読みます)
//すべて負論理,読み取りループ内で反転して取り込んでます
	...
	for(fi=0;fi<2;fi++)
	{
		for(fj=0;fj<6;fj++)
		{
			result_button_press_set[fi] <<= 1;
			if(!SFC_DAT1)	result_button_press_set[fi] |= 0x01;
			SFC_CLK = 0;
			Delay10TCYx(2);
			SFC_CLK = 1;
			Delay10TCYx(2);
