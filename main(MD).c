// USB HID core
//	ver 2	2020/06/05	ProductName修正
...
//MDパッド読み取り
//2ms以上のインターバルを空けて呼び出し
	result_button_press_set[0] = result_button_press_set[1] = 0;

	MD_SEL = 0;
	Delay10TCYx(2);
	if(!MD_D0)//UP
		result_button_press_set[0] |= 0x01;
	if(!MD_D1)//DOWN
		result_button_press_set[0] |= 0x02;
	if(!MD_D4)//A
		result_button_press_set[0] |= 0x04;
	if(!MD_D5)//START
		result_button_press_set[0] |= 0x08;

	MD_SEL = 1;
	Delay10TCYx(2);
	if(!MD_D4)//B
		result_button_press_set[0] |= 0x10;
	if(!MD_D5)//C
		result_button_press_set[0] |= 0x20;
	if(!MD_D2)//LEFT
		result_button_press_set[1] |= 0x01;
	if(!MD_D3)//RIGHT
		result_button_press_set[1] |= 0x02;
	...
	//6ボタンパッドなら追加の4ボタンを読み取り
	if(!(MD_D0 | MD_D1 | MD_D2 | MD_D3))
	{
		MD_SEL = 1;
		Delay10TCYx(2);
		if(!MD_D0)//Z
			result_button_press_set[1] |= 0x04;
		if(!MD_D1)//Y
			result_button_press_set[1] |= 0x08;
		if(!MD_D2)//X
			result_button_press_set[1] |= 0x10;
		if(!MD_D3)//MODE
			result_button_press_set[1] |= 0x20;
		MD_SEL = 0;
		Delay10TCYx(2);
		if(!(MD_D0 & MD_D1 & MD_D2 & MD_D3))
		{
			result_button_press_set[1] &= 0xc3;
		}
	}
